...
import { 
  Gift, MessageCircle, Calendar, ChevronRight, 
  ExternalLink, RefreshCw, MailOpen, ArrowLeft
} from 'lucide-react';
  ...
  if (status === 'loading') {
    return (
      <div className="flex-1 flex flex-col items-center justify-center min-h-[450px]">
        ...
        <span className="text-sm font-semibold text-foreground/60">받은 선물함 불러오는 중...</span>
      </div>
    );
  }
      ...
      <div className="flex-1 bg-background flex flex-col items-center justify-center p-6 text-center">
        <div className="max-w-md w-full bg-white border border-pink-100 rounded-3xl shadow-xl p-8 space-y-6">
          ...
          <p className="text-sm text-foreground/70 break-keep">
            내가 받은 포춘쿠키와 선물(쿠폰) 목록을 확인하시려면 로그인이 필요합니다.
          </p>
          <button
      ...
      <div className="max-w-3xl mx-auto w-full space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-foreground/70 font-medium">
            친구들이 정성껏 보낸 포춘쿠키와 카페 모바일 쿠폰 목록을 확인할 수 있습니다.
          </p>
        </div>
        ...
        <div className="flex bg-white p-1.5 rounded-2xl border border-pink-100 shadow-sm gap-2">
          {[
            ...
            { id: 'unopened', label: '미개봉 쿠키', count: messages.filter(m => !m.is_opened).length },
            { id: 'opened', label: '열어본 편지', count: messages.filter(m => m.is_opened).length },
          ].map((tab) => {
        ...
        {loading ? (
          ...
          <div className="bg-white border border-pink-100 rounded-3xl py-20 text-center space-y-4 shadow-sm">
            ...
            <h3 className="font-bold text-foreground/80 text-base">받은 편지가 아직 없어요</h3>
            <button 
              ...
              {filteredMessages.map((msg, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`bg-white border rounded-3xl shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md ${
                      isUnopened 
                        ? 'border-pink-200 ring-1 ring-primary/5' 
                        : 'border-pink-100'
                    }`}
                  >
                    <div className="p-6 flex flex-col sm:flex-row gap-5 items-start sm:items-center justify-between">
                      ...
                      <div className="flex gap-4 items-center">
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="text-xs font-black text-foreground/40">From.</span>
                      ...
                      <div className="w-full sm:w-auto flex justify-end">
                        {isUnopened ? (
                          <button
                            onClick={() => router.push(`/open/${msg.id}`)}
                            className="w-full sm:w-auto py-3 px-6 bg-gradient-to-r from-primary to-secondary hover:from-pink-600 hover:to-orange-500 text-white font-extrabold text-xs sm:text-sm rounded-2xl shadow-md transition-all flex items-center justify-center gap-1.5 hover:scale-[1.01] cursor-pointer"
                          >
                            <MailOpen className="w-4 h-4" />
                            <span>쿠키 열고 선물 받기</span>
                            <ChevronRight className="w-4 h-4" />
                          </button>
                        ) : (
                          <button
                            onClick={() => router.push(`/open/${msg.id}`)}
                            className="w-full sm:w-auto py-3 px-5 border border-pink-200 hover:bg-pink-50/50 text-foreground/70 font-extrabold text-xs rounded-2xl transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                          >
                            <span>편지 내용 보기</span>
                    ...
                    {!isUnopened && (
                      <div className="px-6 pb-6 pt-1 border-t border-pink-100/50 space-y-4">
                        ...
                        <div className="relative p-4 bg-gradient-to-r from-white to-pink-50/30 border border-pink-100/80 rounded-2xl shadow-inner">
                          <p className="text-foreground text-sm font-bold leading-relaxed break-keep">
                            "{msg.message_content}"
                          </p>
                        </div>
                        ...
                        {/* Reply block */}
                        {msg.reply ? (
                          <div className="flex gap-2 items-start pl-4 border-l-2 border-primary">
                            <div className="space-y-1">
                              <span className="text-[10px] font-black text-primary uppercase tracking-widest block">내 답장 💌</span>
                          ...
                          <div className="flex justify-end pt-1">
                            <button
                              onClick={() => router.push(`/reply/${msg.id}`)}
                              className="inline-flex items-center gap-1 text-[11px] font-extrabold text-primary hover:text-pink-600 transition-colors cursor-pointer"
                            >
